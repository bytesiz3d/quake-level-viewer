...
GenMeshPolygons(std::span<const Poly> polys)
{
	...
	{
		TraceLog(LOG_DEBUG, "[%d] v (%.3f %.3f %.3f), tc (%.3f %.3f), n (%.3f %.3f %.3f)", i, mesh.vertices[3 * i], mesh.vertices[3 * i + 1], mesh.vertices[3 * i + 2], mesh.texcoords[2 * i], mesh.texcoords[2 * i + 1], mesh.normals[3 * i], mesh.normals[3 * i + 1], mesh.normals[3 * i + 2]);
	}
	for (size_t i = 0; i < mesh.triangleCount; i++)
	{
		TraceLog(LOG_DEBUG, "idx (%d %d %d)", mesh.indices[3 * i], mesh.indices[3 * i + 1], mesh.indices[3 * i + 2]);
	}
